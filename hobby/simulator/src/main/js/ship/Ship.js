define(
        [ "Quaternion", "Vector", "ship/Conduit", "ship/Power", "ship/Propulsion", "ship/Sensor", "ship/Storage" ],
        function(Quaternion, Vector, Conduit, Power, Propulsion, Sensor, Storage)
        {
            "use strict";
            ...
            function ReferenceShip(name, environment)
            {
                ...
                // Fuel.
                var storage = new Storage.FuelTank("FuelTank", environment, name,
                        Vector.X_AXIS.multiply(-length / 4.0), Quaternion.ZERO, 10000);

                // Power.
                var power = new Power.FusionReactor("FusionReactor", environment, name, Vector.ZERO, Quaternion.ZERO,
                        1, 200);
                ...
                var mainEngine = new Propulsion.IonEngine("MainEngine", environment, name, aft, forwardQ, 2, 1,
                        mainProducePerTick);
                var portYawAftEngine = new Propulsion.IonEngine("PortYawAftEngine", environment, name, aft, starboardQ,
                        2, 1, retroProducePerTick);
                var starboardYawAftEngine = new Propulsion.IonEngine("StarboardYawAftEngine", environment, name, aft,
                        portQ, 2, 1, retroProducePerTick);
                var dorsalPitchAftEngine = new Propulsion.IonEngine("DorsalPitchAftEngine", environment, name, aft,
                        ventralQ, 2, 1, retroProducePerTick);
                var ventralPitchAftEngine = new Propulsion.IonEngine("VentralPitchAftEngine", environment, name, aft,
                ...
                var portYawForwardEngine = new Propulsion.IonEngine("PortYawForwardEngine", environment, name, forward,
                        portQ, 2, 1, retroProducePerTick);
                var starboardYawForwardEngine = new Propulsion.IonEngine("StarboardYawForwardEngine", environment,
                        name, forward, starboardQ, 2, 1, retroProducePerTick);
                var dorsalPitchForwardEngine = new Propulsion.IonEngine("DorsalPitchForwardEngine", environment, name,
                        forward, dorsalQ, 2, 1, retroProducePerTick);
                var ventralPitchForwardEngine = new Propulsion.IonEngine("VentralPitchForwardEngine", environment,
                        name, forward, ventralQ, 2, 1, retroProducePerTick);

                // Propulsion (roll).
                var dorsalRollPortEngine = new Propulsion.IonEngine("DorsalRollPortEngine", environment, name, port,
                        dorsalQ, 2, 1, retroProducePerTick);
                var ventralRollPortEngine = new Propulsion.IonEngine("VentralRollPortEngine", environment, name, port,
                        ventralQ, 2, 1, retroProducePerTick);
                var dorsalRollStarboardEngine = new Propulsion.IonEngine("DorsalRollStarboardEngine", environment,
                        name, starboard, ventralQ, 2, 1, retroProducePerTick);
                var ventralRollStarboardEngine = new Propulsion.IonEngine("VentralRollStarboardEngine", environment,
                        name, starboard, dorsalQ, 2, 1, retroProducePerTick);
