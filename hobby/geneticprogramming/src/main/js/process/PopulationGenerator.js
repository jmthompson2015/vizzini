define([ "process/GenomeFactory", "process/Population" ], function(GenomeFactory, Population)
{
    "use strict";
    ...
    {
        LOGGER.trace("PopulationGenerator.generate() start");
        ...
        for (var depth = minDepth; depth <= maxDepth; depth++)
        {
            ...
            while (answer.length() < end)
            {
                ...
                var isAdded = answer.maybeAddGenome(tree, duplicatesAllowed);
                count = (isAdded ? 0 : count + 1);

                if (count >= maxTries)
                {
                    LOGGER.debug("Can't generate a unique genome; usingFull ? " + usingFull + " count = " + count);
                    answer.maybeAddGenome(tree, true);
                    count = 0;
                }
            }
        }

        LOGGER.trace("PopulationGenerator.generate() end");
