define(
        [ "ActivationState", "Bearing", "DamageCard", "DamageCardV2", "Difficulty", "Maneuver", "Pilot", "RangeRuler",
                "ShipAction", "ShipBase", "UpgradeCard", "UpgradeType", "Weapon", "process/Action", "process/Selector" ],
        function(ActivationState, Bearing, DamageCard, DamageCardV2, Difficulty, Maneuver, Pilot, RangeRuler,
                ShipAction, ShipBase, UpgradeCard, UpgradeType, Weapon, Action, Selector)
        {
            "use strict";
            function Token(store, pilotKeyIn, agent, upgradeKeysIn)
            {
                    ...
                    return maneuverKeys
                            .map(function(maneuverKey)
                            {
                                ...
                                if (myBearingKey === bearingKey)
                                {
                                    ...
                                    if (!answer) { throw "Unknown maneuver: " + bearingKey + " " + speed + " " +
                                            difficultyKey; }
                                    return answer;
                                }
                                else
                ...
                {
                    return new Weapon(upgrade.name, upgrade.weaponValue, upgrade.ranges, upgrade.firingArcKey,
                            undefined, upgrade.isWeaponTurret, upgrade.value);
                }
