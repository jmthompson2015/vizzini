...
RectanglePath.determineWindingNumber = function(x, y, polygon)
{
    ...
    for (var i = 0; i < n; i += 2)
    {
        ...
        if (points[i + 1] <= y)
        {
            ...
            {
                if (RectanglePath.isLeft(points[i], points[i + 1], points[i + 2], points[i + 3], x, y) > 0) // P left of
                // edge
                {
        ...
        else
        {
            ...
            {
                if (RectanglePath.isLeft(points[i], points[i + 1], points[i + 2], points[i + 3], x, y) < 0) // P right
                // of edge
                {
...
RectanglePath.doPolygonsCollide = function(polygon0, polygon1)
{
    ...
    else
    {
        ...
        for (var y = startY; !answer && y <= endY; y++)
        {
            ...
            {
                // LOGGER.trace(x + "," + y + " isPointInPolygon ? " + RectanglePath.isPointInPolygon(x, y, polygon0)
