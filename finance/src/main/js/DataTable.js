var DataTable = React.createClass(
{
    ...
    createRow: function(symbol, i)
    {
        ...
        if (myData)
        {
            for (var j = 1; j < this.columns.length; j++)
            {
                ...
                cells.push(this.createNumberCell(cells.length, column.key, myData[column.key]));
            }
        }
        else
        {
    ...
    receiveKeyStatisticsData: function(keyStats)
    {
        ...
        myData.freeCashFlow = (keyStats.getFreeCashFlow() ? keyStats.getFreeCashFlow().number : undefined);
        myData.forwardPE = (keyStats.getForwardPE() ? keyStats.getForwardPE().number : undefined);
        myData.dividendYield = (keyStats.getDividendYield() ? keyStats.getDividendYield().number : undefined);
