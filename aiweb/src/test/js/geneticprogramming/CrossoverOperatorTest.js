define(
        [ "Arithmetic", "CountVisitor", "CrossoverOperator", "FragmentVisitor", "Logic", "StringifyVisitor", "Terminal" ],
        function(Arithmetic, CountVisitor, CrossoverOperator, FragmentVisitor, Logic, StringifyVisitor, Terminal)
        {
            "use strict";
            ...
            QUnit.test("crossover() 2", function(assert)
            {
                ...
                LOGGER.debug("result[0] = " + (new StringifyVisitor(result[0])).string());
                LOGGER.debug("result[1] = " + (new StringifyVisitor(result[1])).string());
                assert.equal((new CountVisitor(result[0])).nodeCount(), 3);
                assert.equal((new CountVisitor(result[1])).nodeCount(), 3);
