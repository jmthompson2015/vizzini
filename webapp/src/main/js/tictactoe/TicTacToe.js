function TicTacToe(memory, firstAgent, secondAgent, delayIn, gameOverCallback)
{
    var that = this;
    ...
    this.play = function()
    {
        ...
        agent = firstAgent;
        board = BoardUtilities.createStartBoard();
        addGameRecord(board);
    ...
    function postProcessGame(winner, loser)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < gameRecords.length; i++)
            {
                ...
                var aliases = BoardUtilities.getAliases(board);
                ...
                {
                    // Winner.
                    var whoseMove = BoardUtilities.getWhoseMove(board);
    ...
    function setMove(move)
    {
        ...
        if (0 <= move && move < 9)
        {
            if (BoardUtilities.get(board, move) !== " ")
            {
            ...
            {
                var oldValue = board;
                board = BoardUtilities.move(board, move);
                addGameRecord(board);
                fireBoardChange(oldValue, board);

                if (BoardUtilities.isWinner(board, agent.getTeam()))
                {
                    gameOver(agent, getOpponent(agent));
                }
                else if (BoardUtilities.getMoves(board).length === 0)
                {
                    gameOver();
                }
                else
