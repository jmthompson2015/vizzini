 ...
 */
public final class BinaryEncodeFilter implements InputFilter<double[]>, OutputFilter<double[]>
{
    ...
    public BinaryEncodeFilter(final BinaryEncoder[] encoders)
    {
    ...
    public double[] filter(final double[] input)
    {
        ...
        if (input.length != encoders.length)
        {
            throw new IllegalArgumentException("Expected " + encoders.length + " inputs; received " + input.length);
        }
        ...
        {
            final double[] encoding = encoders[i].encodeAsDoubleArray(input[i]);
    ...
    public double[] reverseFilter(final double[] output)
    {
        ...
        for (int i = 0; i < encoders.length; i++)
        {
            ...
            final double[] subInputs = Arrays.copyOfRange(output, from, to);
