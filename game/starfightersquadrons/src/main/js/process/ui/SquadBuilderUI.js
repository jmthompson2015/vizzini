define(["Pilot", "ShipTeam", "Team", "UpgradeCard", "UpgradeType", "process/Action", "process/SimpleAgent", "process/Squad", "process/TokenFactory", "process/ui/PilotChooser", "process/ui/SquadUI", "process/ui/UpgradeChooser"],
   function(Pilot, ShipTeam, Team, UpgradeCard, UpgradeType, Action, SimpleAgent, Squad, TokenFactory, PilotChooser, SquadUI, UpgradeChooser)
   {
      "use strict";
      var SquadBuilderUI = React.createClass(
      {
         ...
         addActionPerformed: function(event)
         {
            ...
            var oldSquad = this.state.squad;
            var tokens = oldSquad.tokens().slice();
            tokens.push(token);
            var squad = this.createSquad(oldSquad.factionKey(), tokens);
            this.setState(
            {
               squad: squad,
            });
         ...
         createSquad: function(factionKey, tokens)
         {
            var name = "name";
            var year = 2017;
         ...
         removeActionPerformed: function(selected, event)
         {
            var oldSquad = this.state.squad;
            var tokens = oldSquad.tokens().slice();
            var index = tokens.indexOf(selected);
