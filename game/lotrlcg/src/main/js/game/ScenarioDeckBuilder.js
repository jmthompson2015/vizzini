define([ "EnemyCard", "LocationCard", "QuestCard", "TreacheryCard", "game/EnemyToken", "game/LocationToken",
        "game/QuestToken", "game/ScenarioDeck", "game/TreacheryToken" ], function(EnemyCard, LocationCard, QuestCard,
        TreacheryCard, EnemyToken, LocationToken, QuestToken, ScenarioDeck, TreacheryToken)
{
    "use strict";
    var DeckBuilders = [];
    ...
    var CoreSetPassageThroughMirkwoodDeckBuilder = new ScenarioDeckBuilder("Passage Through Mirkwood (Core #1)", 2011,
            "Passage Through Mirkwood", function()
            {
                ...
                addEnemy(encounterTokens, EnemyCard.CHIEFTAIN_UFTHAK, 1);
    ...
    function addEnemy(array, cardKey, count)
    {
        ...
        {
            array.push(new EnemyToken(cardKey));
        }
    }
    ...
    function addLocation(array, cardKey, count)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            array.push(new LocationToken(cardKey));
        }
    }
    ...
    function addQuest(array, cardKey)
    {
        ...
        array.push(new QuestToken(cardKey));
    }
    ...
    function addTreachery(array, cardKey, count)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            array.push(new TreacheryToken(cardKey));
        }
    }
