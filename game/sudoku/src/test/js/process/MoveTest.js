define(["PuzzleFormat", "SudokuToGo", "process/Move"],
    function(PuzzleFormat, SudokuToGo, Move)
    {
        ...
        QUnit.test("batchRemoveCandidates()", function(assert)
        {
            ...
            assert.equal(puzzle.get(indices[0]).candidates().length, 4);
            ...
            // Verify.
            assert.ok(result);
            assert.equal(result.get(indices[0]).candidates().length, 3);
        ...
        QUnit.test("removeCellCandidate()", function(assert)
        {
            ...
            assert.equal(puzzle.get(index).candidates().length, 4);
            ...
            // Verify.
            assert.equal(result.get(index).candidates().length, 3);
        ...
        QUnit.test("setCellValue()", function(assert)
        {
            ...
            assert.equal(puzzle.get(index).candidates().length, 3);
